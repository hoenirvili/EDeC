  ...
  PROCEDURE insertUser (
    ...
    BEGIN
      ...
      WHEN WRONG_USERNAME_FORMAT THEN
        IF LENGTH(v_username)>25 THEN DBMS_OUTPUT.PUT_LINE('Username too long');END IF;
        IF LENGTH(v_username)<6 THEN DBMS_OUTPUT.PUT_LINE('Username too short');
        ELSE DBMS_OUTPUT.PUT_LINE('Wrong username: invalid character used '||REGEXP_SUBSTR(v_username,'[^a-zA-Z0-9\._-]')); END IF;     
